"""Write the 投手主客 tab: each team's starter / bullpen / total ERA by venue.
...
summarising the season by team, segment and venue.

The tab is rewritten whole on every run rather than appended to, so the daily
job can refresh it after any day's games without leaving a stale row behind.
`baseball.npb_pitching_splits` holds the arithmetic; everything here is the
sheet: where it lives, and how it is painted.
"""
